# Phase S9 — Link pass. SONNET session. Sequential, ONLY after S5–S8 are all merged.

Read ONLY: this file, `PLAN.md` §1, §4, §6.S9, the phase table and §9 index,
and `docs/log/s5.md`, `s6.md`, `s7.md`, `s8.md`. Execute under the autonomy
protocol §4.

Owns: ESLint + Prettier config files, `package.json` (`lint`, `format`,
`verify`), `.github/workflows/ci.yml` (add the lint step), the repo-wide
autofix commit, `src/components/Header.tsx` (final nav incl. `/youtube/admin`
for owners), any component S5 and S6 both shipped (dedupe), `KNOWN-ISSUES.md`,
...
- Header nav: Content, Inbox, Digest, Topics, Marks, Sources, Ingest,
  Admin (owner only). Mount S6's `IdeaActions`/`IdeaStatusTabs` on
...
report to Anton: what shipped per phase (PR links), live URL, PLAN §7 items
still unchecked, and exact numbered manual steps (Vercel env, probe run,
decisions-needed answers). No further sessions.
