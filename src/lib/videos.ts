 ...
 *
 * `takeaways` and `ideas` are JSON columns; Postgres will not ILIKE them
 * directly, hence the `::text` cast. That searches the raw JSON — keys and
 * punctuation included — which is exactly what §9's "raw JSON of
 * takeaways/ideas" asks for and is good enough for a private tool. Nothing
 * here is interpolated: `like()` and the `sql` template both parameterise
 * their values. ILIKE rather than LIKE for case-insensitive matching, since
 * Postgres's default collation is case-sensitive (MySQL's was not).
 */
function matchesQuery(q: string) {
  const needle = `%${q}%`;
  return or(
    sql`${videos.title} ilike ${needle}`,
    sql`exists (
      select 1 from ${analyses} a
      where a.video_id = ${videos.id}
        and a.status = 'ok'
        and (
          a.summary ilike ${needle}
          or a.takeaways::text ilike ${needle}
          or a.ideas::text ilike ${needle}
        )
    )`,
  );
}
...
export async function listDigestVideos(query: DigestQuery): Promise<DigestPage> {
  ...
  const rows = await db
    ...
    // nullable and both repeat, and without a unique last key the database is
    // free to order ties differently per page, which drops or duplicates rows
    // across LIMIT/OFFSET boundaries.
