...
async function syncOne(
  ...
  // against the unique index. `onConflictDoNothing` deliberately leaves `name`
  ...
  await tx
    ...
    .onConflictDoNothing({ target: lookup.slug });
 ...
 * under whichever labels it has since abandoned. `distinct on` is Postgres's
...
export async function listContentTypes(): Promise<Array<{ contentType: string; videoCount: number }>> {
  const distinctNewest = db
    .selectDistinctOn([analyses.videoId], {
      ...
      contentType: analyses.contentType,
    })
    .from(analyses)
    .where(and(eq(analyses.status, "ok"), isNotNull(analyses.contentType)))
    .orderBy(analyses.videoId, sql`${analyses.id} desc`)
    .as("newest");

  const rows = await db
    .select({
      contentType: distinctNewest.contentType,
      videoCount: sql<number>`count(*)`,
    })
    .from(distinctNewest)
    .groupBy(distinctNewest.contentType)
    .orderBy(sql`count(*) desc`);

  return rows
    .filter((r): r is { contentType: string; videoCount: number } => Boolean(r.contentType))
